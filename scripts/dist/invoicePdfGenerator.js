 ...
 */
function generateInvoicePdf({ products, meta, role, userOrders, customerName = 'Customer User', autoDownload = true, }) {
    ...
    const drawPage1Header = () => {
        ...
        doc.text(`${isAdmin ? 'STATEMENT' : 'INVOICE'} #${meta.invoiceNumber || 'SMH-001'}`, metaBoxX + 3, metaBoxY + 5.5);
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(7.5);
    ...
    const drawTableHeader = (isContinuation = false) => {
        if (isContinuation) {
            ...
            doc.setFontSize(7.5);
            doc.setTextColor(100, 116, 139);
            doc.text(`${isAdmin ? 'INVENTORY STATEMENT' : 'INVOICE'} #${meta.invoiceNumber || 'SMH-001'} (Continued)`, marginLeft, currentY - 2.5);
        }
    ...
    doc.setFontSize(8);
    doc.setTextColor(15, 23, 42);
    doc.text(isAdmin ? 'TOTAL INVENTORY VALUE:' : 'ORDER GRAND TOTAL:', colPriceX + colPriceWidth - 2.5, currentY + 5.8, { align: 'right' });
