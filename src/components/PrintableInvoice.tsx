...
export function PrintableInvoice({
  ...
  adminGrandTotal,
  totalAdminStock,
  totalUserOrderedUnits,
}: PrintableInvoiceProps) {
  const isAdmin = role === 'admin';

  // For users, show items that have been ordered (or all catalog items if none selected yet)
  const itemsToPrint = isAdmin 
    ? products 
    : products.filter(p => (userOrders[p._id] || 0) > 0);

  const displayProducts = !isAdmin && itemsToPrint.length === 0 ? products : itemsToPrint;

  const currentGrandTotal = isAdmin ? adminGrandTotal : userGrandTotal;
  ...
  return (
    <div className="hidden print:block fixed inset-0 bg-white z-[9999] p-8 text-black font-sans">
      ...
      {/* Summary Chips */}
      <div className="grid grid-cols-3 gap-4 mb-6 p-3 bg-slate-50 border border-slate-200 rounded text-center text-xs">
        <div>
        ...
        <div>
          ...
          <span className="text-base font-bold text-slate-900">
            {isAdmin ? totalAdminStock.toLocaleString() : totalUserOrderedUnits.toLocaleString()}
          </span>
        </div>
        <div>
          <span className="text-slate-500 block font-medium">
      ...
      <table className="w-full border-collapse text-xs border border-slate-300">
        <thead>
          <tr className="bg-slate-100 border-b border-slate-300 font-bold text-slate-800 text-left">
            ...
            <th className="py-2 px-3 text-right w-28 border-r border-slate-300">Price ({meta.currency})</th>
            {isAdmin ? (
              <th className="py-2 px-3 text-center w-24 border-r border-slate-300">Warehouse Stock</th>
            ) : (
              <th className="py-2 px-3 text-center w-24 border-r border-slate-300">Ordered Qty</th>
            )}
          ...
          {displayProducts.map((p, idx) => {
            ...
            return (
              <tr key={p._id} className="border-b border-slate-200">
                ...
                <td className="py-1.5 px-3 font-medium text-slate-900 border-r border-slate-200">
                  {p.name}
                </td>
        ...
        <tfoot>
          <tr className="bg-slate-100 font-bold border-t-2 border-slate-800 text-slate-900">
            ...
            <td className="py-3 px-3 text-center text-xs">
              {isAdmin ? `${totalAdminStock} units` : `${totalUserOrderedUnits} units`}
            </td>
            <td className="py-3 px-3 text-right text-sm font-black text-slate-950">
              {meta.currency}{currentGrandTotal.toLocaleString()}
            </td>
          </tr>
        </tfoot>
      ...
      <div className="mt-16 pt-8 border-t border-slate-300 flex justify-between items-center text-xs text-slate-500">
        <div>
